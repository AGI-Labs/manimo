...
class AIAgent:
    def __init__(self, agent_path, model_name="r3m_stacking_newdata2.ckpt", goal_image=None):
        ...
        self.goal_image = imageio.imread(goal_image)
    ...
    def get_raw_imgs_and_obs(self, env_obs, IMG_SIZE=256):
        ...
        resized_imgs.append(self.goal_image)
...
class Eval(BaseCallback):
    ...
    def __init__(self, logger, agent_paths, goal_image=None):
        super().__init__(logger)
        self.logger = logger
        self.ai_agents = [
            AIAgent(Path(agent_path).parent, Path(agent_path).name, goal_image=goal_image)
...
def main():
    ...
    eval_callback = Eval(logger, args.agent_paths, args.goal_image)
