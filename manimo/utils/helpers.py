...
class StateManager:
    def __init__(self):
        """
        Initialize the StateManager with an empty dict to store
        button states.
        """
        self.button_states = {}

    def handle_state(self, buttons, button_key, callback, *args):
        """Handles the state of a button.

        Args:
            buttons: A dictionary of button states.
            button_key: The key of the button in the dictionary.
            callback: A function to be called when the button state changes.
            args: Additional arguments to be passed to the callback function.
        """
        if buttons:
            # Initialize the button state if not present in the button_states
            #  dictionary
            ...
            # If the button is released and the previous state was INPROGRESS,
            # call the provided function
            # and set the state back to OFF
            elif (
                not button_toggle
                and self.button_states[button_key] == ButtonState.INPROGRESS
            ):
                self.button_states[button_key] = ButtonState.OFF
